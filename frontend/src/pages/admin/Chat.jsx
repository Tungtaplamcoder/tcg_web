...
import {
  ...
  CheckCircle2, Wifi, WifiOff, Headset, ChevronDown, Lock, Unlock
} from 'lucide-react';
...
const AdminChat = () => {
  ...
  const handleUpdateStatus = async (status) => {
    ...
    try {
      ...
      setError(err.response?.data?.error?.message || 'Failed to update status. Please try again.');
    } finally {
      ...
      <div className="w-full lg:w-80 bg-white dark:bg-slate-900 rounded-lg shadow overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto">
          {loadingRooms ? (
            ...
            <div className="text-center py-8 text-gray-500 dark:text-slate-400 text-sm">Chưa có phòng chat nào.</div>
          ) : (
          ...
          <>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-50 dark:bg-slate-900">
              {loadingMessages ? (
                ...
                <div className="text-center text-gray-500 dark:text-slate-400 py-8">Chưa có tin nhắn trong phòng này.</div>
              ) : (
                messages.map((msg) => {
            ...
            <form onSubmit={handleSend} className="p-3 border-t border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900">
              ...
              <div className="mt-1 text-xs text-gray-400 dark:text-slate-500 flex items-center gap-1.5">
                {socketConnected ? (
